...
# k = 0
# for i in open('task9/task9(5).txt'):
#     n = sorted([int(x) for x in i.split()])
#     if n[0]*5 < n[1] + n[2] +n[3]: 
#         if (n[0] * n[3] == n[1] * n[2]) or (n[0] * n[2] == n[1] *n[3]) or (n[0] * n[1] == n[2] * n[3]):
#             k+=1
# print(k)


k = 0
for s in open('task9/9(7).txt'):
    n = [int(x) for x in s.split()]
    k +=1
    n1 =[x for x in n if n.count(x) == 2]
    n2 =[x for x in n if n.count(x) == 1]
    if (len(n1) == 4 and len(n2) == 2):
            if sum(n2) <= (sum(set(n1))):
                  print(sum(n))
